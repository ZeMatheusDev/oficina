				...
				<div class="flex ml-4  grid grid-cols-8 gap-8 max-md:grid-cols-1">
					...
					<div>
						<span class="p-float-label">
							...
							<label for="valor_para_venda" class="text-sm">Valor da moto</label>
						</span>
					</div>

					<div>
						<span class="p-float-label">
							<InputText v-model="form2.observacoes" id="observacoes" type="text" class="w-full" required />
		...
		<SlideUpDown v-model="toggleFilter" :duration="300">
			...
			<div class="text-sm text-gray-500 flex items-center">
				<div class="space-x-6 flex ml-4 grid grid-cols-6 gap-6 max-md:grid-cols-1">
					...
					<div class="flex items-center space-x-2">
						<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['valor_para_venda']" />
						<span>Valor da moto</span>
					</div>

					<div class="flex items-center space-x-2">
						<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['observacoes']" />
						...
						<thead class=" mb-24">
							<tr class="text-gray-500 font-bold select-none" @click="setParams">
								...
								<th v-if="formColumns.columns.valor_para_venda" scope="col"
									...
									<div class="flex">
										<span class="group-hover:text-indigo-800">Valor da moto</span>
										<svg xmlns="http://www.w3.org/2000/svg"
											class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
											viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
											<path stroke-linecap="round" stroke-linejoin="round"
								...
								<th v-if="formColumns.columns.observacoes" scope="col"
									class="px-4 text-sm cursor-pointer text-center border-r group"
									@click="orderBy = { column: 'observacoes', sorting: sortTable(sortVal.observacoes) ? sortVal.observacoes = 1 : sortVal.observacoes = 0 }">
									<div class="flex">
										<span class="group-hover:text-indigo-800">Observações</span>
										<svg xmlns="http://www.w3.org/2000/svg"
											class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
											viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
											<path stroke-linecap="round" stroke-linejoin="round"
						...
						<tbody class="divide-y divide-gray-200 bg-white">
							<tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
								...
								<td v-if="formColumns?.columns?.observacoes"
									...
									<div class="flex items-center">
										<div>
											<div class="font-medium text-gray-900">{{ data?.observacoes }}</div>
										</div>
									</div>
								</td>
								...
								<td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6" style="width: 5%;">
									<Comprar 
									:routeComprar="route('comprando', { id: data?.token })"
									/>	
								</td>
							</tr>
...
const formColumns = useForm({
	...
	columns: {
		...
		placa: validateColumnsVisibility("cor"),
		ano: validateColumnsVisibility("cor"),
...
function setParams() {
 ...
 function FiltroAvancadoAplica() {
	...
	!orderBy.value?.length ? (data.orderBy = orderBy?.value) : "";  
	submitForm.post(route("list.ConfigMotos"), {
	  replace: false,
	  data,
	  onSuccess: () => {
